...
RT.analysis %>% group_by(Block, Condition, Vp) %>% summarise(Latencies = mean(Latencies)) %>% #one value per participant => correct calculation of standard error
  ...
  ggplot(aes(x = Block, y = Latencies, shape=Condition, group=Condition)) +
...
# cor.test(n2pc_TraitPow$TraitPow, n2pc_TraitPow$Diff_P78_NA, method=c("pearson"))

# Interaction trait power + power condition on n2pc
...
n2pc_TraitPow <- left_join(n2pc, PreStud_FINAL_lim, by = join_by("Vp" == "subject_2"), relationship = "many-to-many") %>% 
...
# n2pc.analysis_Trait = n2pc_TraitPow %>% filter(subset == "all", distractors != "all", Vp %in% include) %>% select(-subset)
