...
##### N2pc for Block 1; inlcuded are all NA and NA; only if 50 valid trials (per Block?)
...
path.N2pc_Block1 = "N2pc_Block1_50Segm_only_compl_NA_AN_all_electrodes_modif.dat" %>% paste0(path,.)

n2pc_1_diss_wide = read.delim(path.N2pc_Block1, sep=" ", header = F) %>% t() %>% row_to_names(row_number = 1) %>% as.data.frame
rownames(n2pc_1_diss_wide) <-c(1:400)
vec <- c(-99:300) 
n2pc_1_diss_wide <- cbind(n2pc_1_diss_wide, Time = vec)
n2pc_1_diss_wide <- n2pc_1_diss_wide[,c(11,15)]
...
##### N2pc for Block 2; inlcuded are all NA and NA; only if 50 valid trials (per Block?)
path = "G:/Data_PCs_ETH/Office PC/OB Sever/EEG Study_Dot Probe Task_RW_Final/EEG Study_Dot Probe Task_RW/Dot Probe Task 2_Data/GitHub/N2pc_Powerlessness_Project/Generic Exports Block 1 and 2/"
path.N2pc_Block2 = "N2pc_Block2_50Segm_only_compl_NA_AN_all_electrodes_modif.dat" %>% paste0(path,.)

n2pc_1_diss_wide = read.delim(path.N2pc_Block2, sep=" ", header = F) %>% t() %>% row_to_names(row_number = 1) %>% as.data.frame
rownames(n2pc_1_diss_wide) <-c(1:400)
vec <- c(-99:300) 
n2pc_1_diss_wide <- cbind(n2pc_1_diss_wide, Time = vec)
n2pc_1_diss_wide <- n2pc_1_diss_wide[,c(11,15)]
