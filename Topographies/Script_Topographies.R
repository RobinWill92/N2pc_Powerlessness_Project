...
eegUtils::topoplot(eeg.map_Block2.m, 
                   #chanLocs = chanlocs, #there seems to be a default chanLocs file that recognizes electrode names in the 10-20 notation
                   palette = "viridis", contour = FALSE, interp_limit = "head", highlights=c("P7", "P8")) #chan_marker="name"
...
eeg.map_Block2.AnNa = eeg.map_Block2 %>% 
  mutate(electrodeNum = electrodeNum + ifelse(condition=="NA", 0, 1),
         electrode = paste0(electrodeChar, electrodeNum))
eegUtils::topoplot(eeg.map_Block2.AnNa, 
                   #chanLocs = chanlocs, #there seems to be a default chanLocs file that recognizes electrode names in the 10-20 notation
                   palette = "viridis", contour = FALSE, interp_limit = "head", highlights=c("P7", "P8")) #chan_marker="name"
...
eeg.map_both = eeg.map_Block1.m %>% mutate(Block=1) %>% bind_rows(eeg.map_Block2.m %>% mutate(Block=2))
eeg.map_both_FINAL = eeg.map_both %>% summarise(amplitude = mean(amplitude), .by = c("electrode", "electrodeChar", "electrodeNum")) %>% 
  mutate(Block="Average") %>% bind_rows(eeg.map_both_FINAL) %>% mutate(Block = Block %>% as_factor())
#N2pc
eegUtils::topoplot(eeg.map_both_FINAL, 
                   #chanLocs = chanlocs, #there seems to be a default chanLocs file that recognizes electrode names in the 10-20 notation
                   palette = "viridis", contour = FALSE, interp_limit = "head", highlights=c("P7", "P8")) #chan_marker="name"
